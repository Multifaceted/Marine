...
def plot_average(model, df, n_predictions):
    ...
    plt.plot(df["Time_rounded"], m, 'r', label='ensemble means' if i == 0 else None, linewidth=1.,color="red")
    plt.plot(df["Time_rounded"], m + 2 * s, 'g', linewidth=0.5, label='ensemble means + 2 ensemble stdev' if i == 0 else None, color="green");
    plt.plot(df["Time_rounded"], m - 2 * s, 'g', linewidth=0.5, label='ensemble means - 2 ensemble stdev' if i == 0 else None, color="green");
